use group_action_circuit::GroupActionCircuit;

/// Aggregate gate and qubit statistics from a fully-constructed circuit.
pub fn print_resource_table(circuit: &GroupActionCircuit) {
    ...
    println!(
        "│ QFT Hadamards (est.)     │ {:<12} │",
        summary.qft_hadamards_estimated
    );
    println!(
        "│ QFT rotations (est.)     │ {:<12} │",
        summary.qft_rotations_estimated
    );
    println!(
        "│ Point additions          │ {:<12} │",
