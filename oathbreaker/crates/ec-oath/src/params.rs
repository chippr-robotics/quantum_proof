...
/// field (the curve order ~ p +/- 2*sqrt(p) by Hasse's theorem, so for p ~ 2^64
...
pub struct RawCurveParams {
    ...
    pub tier: String,
    #[allow(dead_code)]
    pub embedding_degree: Option<u128>,
    #[allow(dead_code)]
    pub discriminant: Option<u128>,
}

impl RawCurveParams {
    /// Convert raw JSON parameters to typed CurveParams.
