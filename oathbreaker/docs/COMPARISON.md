...
| What's proven | Point addition only | Full Shor circuit end-to-end |
| Classically verifiable | No (256-bit ECDLP infeasible) | Yes (Pollard's rho in minutes) |
...
| Häner et al. | 2020 | 2,048 | 126B | Improved circuits |
...
| Oathbreaker (projected) | 2026 | TBD | TBD | Oath-64 → 256-bit scaling |

## Key Differences

### vs. Roetteler/Häner
- We implement and measure the full circuit, not just estimate
- Our Oath-64 measurements provide a concrete anchor for scaling projections
- We include the QFT and phase estimation, not just the arithmetic

### vs. Litinski
- Litinski's 50M Toffoli uses measurement-based uncomputation (requires active error correction)
- Our circuit uses standard reversible uncomputation (works on any gate-model quantum computer)
- Both approaches are valid — they target different hardware models

### vs. Google
- Google proved correctness of a single subroutine (point addition) at 256 bits
- We prove the entire Shor circuit end-to-end on the Oath-64 curve
- Our proof is independently verifiable via classical ECDLP solving
- Our circuit is fully transparent and open-source
- We provide a standardized benchmark scale (Oathbreaker Scale)
